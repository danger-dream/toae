...
import { configuration as conf, generateOcrConfig, generateTransConfig } from '../Configuration'
...
import { LanguageList, LanguageZh, plugins as TransPlugins } from '../Plugins/Translator'
...
const imageTrans = computed(() => conf.trans_services.flatMap(item => {
	const service = TransPlugins.find(plugin => plugin.name === item.name)
	if (!item.id || !item.enable || !item.transVerify || !service?.Translate) return []
	return [{ id: item.id, label: item.label || service.label || service.name }]
}))
const imageOcr = computed(() => conf.ocr_services
	.map(generateOcrConfig)
	.filter((item): item is NonNullable<typeof item> => Boolean(item))
	.filter(item => supportsPositionedOcr(item.name, item.params)))

function supportsPositionedOcr(name: string, params?: Record<string, any>): boolean {
	if (name === 'tencent') return true
	if (name === 'baidu') return true
	return name === 'custom' && String(params?.resType ?? 'text') === 'json'
}
