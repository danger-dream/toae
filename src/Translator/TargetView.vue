...
async function playAudio(url: string) {
	...
	try {
		let source = url
		if (url.startsWith('toae-audio:')) {
			const provider = window.toae.provider
			if (!provider) return
			const audio = await provider.fetchAudio(url.slice('toae-audio:'.length))
			objectUrl = URL.createObjectURL(new Blob([audio.bytes], { type: audio.mime }))
			source = objectUrl
		}
		const player = new Audio(source)
		if (objectUrl) {
			const release = () => { URL.revokeObjectURL(objectUrl); objectUrl = '' }
