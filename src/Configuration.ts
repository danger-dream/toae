...
export class Configuration {
	...
	/** 截图翻译是否直接生成译图并复制到剪切板 */
	public image_translate_enabled = false
	/** 图片直译首选的结构化 OCR 服务实例 */
	public image_translate_ocr_service = ''
	/** 图片直译首选的文本翻译服务实例 */
	public image_translate_trans_service = ''
...
export function generateTransConfig(item: IServiceConfig): ITransServiceConfig | undefined {
	...
	if (configuration.only_dict) {
		if (!plugin.Dict || !item.dictVerify) return undefined
	} else if (!item.transVerify && !(plugin.Dict && item.dictVerify)) {
		return undefined
	}
	temp.service = plugin
	return temp
