...
import type { CaptureFramePayload } from '../contracts'
...
onMounted(() => {
	...
	let preview_url = ''
	const capture = window.toae.capture
	if (!capture) throw new Error('截图窗口缺少 capture preload API')
	let selecting = false
	let canvas_rect = []
	let in_rect = false
	let moving = false
	...
	main_canvas.style.width = clip_canvas.style.width = '100vw'
	main_canvas.style.height = clip_canvas.style.height = '100vh'
	main_canvas.width = clip_canvas.width = Math.max(1, Math.round(window.innerWidth * window.devicePixelRatio))
	main_canvas.height = clip_canvas.height = Math.max(1, Math.round(window.innerHeight * window.devicePixelRatio))
// 生成取色器
	let inner_html = ''
	...
	let action_type = ''
	capture.onFrame((payload: CaptureFramePayload) => {
		if (!payload?.meta?.sessionId || !(payload.bytes instanceof ArrayBuffer) || payload.bytes.byteLength === 0) return
		debug(`截图帧已接收: ${payload.bytes.byteLength} bytes`)
		current_session_id = payload.meta.sessionId
		action_type = payload.meta.action
		if (preview_url) URL.revokeObjectURL(preview_url)
		preview_url = URL.createObjectURL(new Blob([payload.bytes], { type: payload.meta.mime }))
		const img = new Image()
		img.src = preview_url
		...
		img.onload = async function() {
			...
			ratio = img.width / Math.max(1, window.innerWidth)
			main_canvas.getContext('2d')!.drawImage(img, 0, 0)
			debug(`截图帧已渲染: ${img.width}x${img.height}`)
	...
	async function winClose(cancel = true) {
		...
		await hideWindow()
		if (cancel && session_id) await capture.cancel(session_id).catch(() => undefined)
		if (preview_url) {
	...
	clip_canvas.ondblclick = async (e) => {
		is_in_clip_rect(e)
		if (!in_rect || !current_session_id) return
		debug('invoke clip double click ok:' + action_type)
		const session_id = current_session_id
		const scale_x = main_canvas.width / Math.max(1, clip_canvas.offsetWidth)
		const scale_y = main_canvas.height / Math.max(1, clip_canvas.offsetHeight)
		const roiDip = {
			x: final_rect[0] / scale_x,
			y: final_rect[1] / scale_y,
			width: final_rect[2] / scale_x,
			height: final_rect[3] / scale_y
		}
		await winClose(false)
		await capture.confirm({ sessionId: session_id, roiDip }).catch(error => debug('截图确认失败: ' + String(error)))
	}
		...
		for (let i = 0, len = color.length; i < len; i += 4) {
			...
			if (!(x0 <= xx && xx <= x1 - 1 && y0 <= yy && yy <= y1 - 1) && ii !== (color.length / 4 - 1) / 2) {
				...
				try {
					...
					const the_text_color = [String(color.hex(undefined)), color.isLight() ? '#000' : '#FFF']
					...
					main_el.innerText = String(Color(color_g).hex())
				} catch {
				}
			} else {
